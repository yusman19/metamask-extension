...
describe('Actions', () => {
  ...
  describe('#markNoticeRead', () => {
    let markNoticeReadSpy, showNoticeSpy, clearNoticesSpy
    const notice = {
      id: 0,
      read: false,
      date: 'test date',
    ...
    beforeEach(() => {
      ...
      showNoticeSpy = sinon.spy(actions, 'showNotice')
      clearNoticesSpy = sinon.spy(actions, 'clearNotices')
    })

    afterEach(() => {
      markNoticeReadSpy.restore()
      showNoticeSpy.restore()
      clearNoticesSpy.restore()
    })
    ...
    it('calls markNoticeRead in background', () => {
      ...
      store.dispatch(actions.markNoticeRead(notice))
        .then((done) => {
          assert(showNoticeSpy.calledOnce)
          assert(showNoticeSpy.calledWith(notice))
    ...
    it('clears notices when markedNotice is called with no notice', () => {
      const store = mockStore()

      store.dispatch(actions.markNoticeRead())
        .then((done) => {
          assert(clearNoticesSpy.calledOnce)
          done()
        })
    })
